...
def categorize_intensity(mean_intensity: float) -> str:
    if mean_intensity is None or np.isnan(mean_intensity):
        return "None"
    if (-1.64 < mean_intensity) and (mean_intensity <= -1.28):
        return "Moderate"
    if (-1.96 < mean_intensity) and (mean_intensity <= -1.64):
        return "Severe"
    if mean_intensity <= -1.96:
        return "Extreme"
    return "None"
    ...
    for i in range(len(index_np)):
        ...
        if np.isnan(val):
            if in_event:  
                ...
                if len(segment) > 0:  
                    mean_intensity = segment.mean()
                    category = categorize_intensity(mean_intensity)
                    droughts.append([
...
def bucketize_events(ev_list):
    buckets = {"Moderate": [], "Severe": [], "Extreme": []}
    for e in ev_list:
        cat = e[4]
        if cat in buckets:
...
def calculate_category_metrics(events, hist_events):
    ev_cat = bucketize_events(events)
    hist_cat = bucketize_events(hist_events)
    ...
    for cat in ["Moderate", "Severe", "Extreme"]:
        ...
        metrics[f'frequency_{cat.lower()}'] = len(ev)
        metrics[f'hist_frequency_{cat.lower()}'] = len(hev)
        metrics[f'ratio_frequency_{cat.lower()}'] = len(ev) / len(hev) if len(hev) > 0 else np.nan
        metrics[f'diff_frequency_{cat.lower()}'] = len(ev) - len(hev)
        
        dur_mean = mean_or_nan([e[2] for e in ev])
        h_dur_mean = mean_or_nan([e[2] for e in hev])
        metrics[f'duration_mean_{cat.lower()}'] = dur_mean
        metrics[f'hist_duration_mean_{cat.lower()}'] = h_dur_mean
        metrics[f'ratio_duration_mean_{cat.lower()}'] = dur_mean / h_dur_mean if not np.isnan(dur_mean) and not np.isnan(h_dur_mean) and h_dur_mean > 0 else np.nan
        metrics[f'diff_duration_mean_{cat.lower()}'] = dur_mean - h_dur_mean if not np.isnan(dur_mean) and not np.isnan(h_dur_mean) else np.nan
        
        inten_mean = mean_or_nan([(-1.0 * e[3]) for e in ev])
        h_inten_mean = mean_or_nan([(-1.0 * e[3]) for e in hev])
        metrics[f'intensity_mean_{cat.lower()}'] = inten_mean
        metrics[f'hist_intensity_mean_{cat.lower()}'] = h_inten_mean
        metrics[f'ratio_intensity_mean_{cat.lower()}'] = inten_mean / h_inten_mean if not np.isnan(inten_mean) and not np.isnan(h_inten_mean) and h_inten_mean > 0 else np.nan
        metrics[f'diff_intensity_mean_{cat.lower()}'] = inten_mean - h_inten_mean if not np.isnan(inten_mean) and not np.isnan(h_inten_mean) else np.nan
        
        drought_days = sum_or_nan([e[2] for e in ev])
        h_drought_days = sum_or_nan([e[2] for e in hev])
        metrics[f'drought_days_{cat.lower()}'] = drought_days
        metrics[f'hist_drought_days_{cat.lower()}'] = h_drought_days
        metrics[f'ratio_drought_days_{cat.lower()}'] = drought_days / h_drought_days if not np.isnan(drought_days) and not np.isnan(h_drought_days) and h_drought_days > 0 else np.nan
        metrics[f'diff_drought_days_{cat.lower()}'] = drought_days - h_drought_days if not np.isnan(drought_days) and not np.isnan(h_drought_days) else np.nan
...
for model in tqdm(all_models, desc="Models"): 
    for scenario in scenarios: 
        ...
        for WS in window_sizes: 
            ...
            wind_speed_100_all = wind_speed_all * ((100 / 10) ** ((0.143 + 0.11) / 2)) 
            W_pot_all = xr.where( 
            ...
            output_dir = "model_outputs/NewVersion/CorrectedFormulaDiffDroughts" 
            os.makedirs(output_dir, exist_ok=True)
                    ...
                    for i in range(fut_PV_smoothed.lat.size): 
                        for j in range(fut_PV_smoothed.lon.size): 
                            try:
                                ...
                                cat_metrics = calculate_category_metrics(events, hist_events)
                                ...
                                result_entry.update(cat_metrics)
                                
                                current_wl_grid_results_buffer.append(pd.DataFrame([result_entry])) 
                                    
                                del idx_vals, t_vals, start_dt, end_dt, mask, events, hist_events, hist_data_for_grid, hist_pv_series_stored, hist_wind_series_stored, result_entry, cat_metrics
